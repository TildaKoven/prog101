# edited to do exercise 2 of prog101

# Fill in the blanks so the answers are all 2. Use help for unfamiliar functions
# if necessary.
log10(100)
sqrt(4)
ceiling(1.1)

# Find the functions for cosine and sine. What are the cosine and sine of 0?
cos(0) <- 1
sin(0) <- 0
# What do you think the following functions do? Just go off the name, don't look
# them up.
read.csv()
# reads the comma separated values
boxplot()
# creates a box plot of the arguments
startsWith()
# tells you what the first letter or term of the argument is
